...
contract UniswapV3DirectSwapper is DeploymentAddresses, TransferHelper {
    ...
    */
    ...
    function _swapV3(
        ...
        // workaround for StackTooDeep errors
        address _tokenIn = tokenIn;
        address _tokenOut = tokenOut;
        uint256 _amountIn = amountIn;
        int256 ___amountIn = int256(_amountIn);
        address  __tokenIn = tokenIn;
        uint24 _fee = fee;
        uint32 _slippageBps = slippageBps;
        uint160 sqrtPriceLimitX96;
        bool zeroForOne;
        
        (poolUsed, _fee, sqrtPriceLimitX96, zeroForOne) = parsePool(_tokenIn, _tokenOut, _fee);
        address _poolUsed = poolUsed;
        uint160 _sqrtPriceLimitX96 = sqrtPriceLimitX96;

        // handle incoming funds
        if (msg.value > 0 && _tokenIn == wrappedEther) {
            ...
            if (pullIn) {
                if (tokenAllowance(_tokenIn, msg.sender, address(this)) < _amountIn) {
                    revert InsufficientAllowance(__tokenIn, msg.sender, _amountIn);
                }
                safeTransferFrom(_tokenIn, msg.sender, address(this), _amountIn);
            }
        }
        ...
        if (_slippageBps > 0) {
            ...
            amountOutExpected = quoteV3View(_tokenIn, _tokenOut, _fee, _amountIn, sqrtPriceLimitX96);
            minAmountOut = (amountOutExpected * (10000 - _slippageBps)) / 10000;
        }

        // sanity cast
        if (_amountIn > uint256(type(int256).max)) revert AmountTooLargeForInt256();
        // Workaround for StackTooDeep errors
        address ___tokenIn = _tokenIn;
        address __tokenOut = _tokenOut;
        uint24 __fee = _fee;
        int256 ____amountIn = ___amountIn;
       

        // perform swap on poolUsed
        (int256 amount0, int256 amount1) = IUniswapV3Pool(_poolUsed).swap(
            address(this),
            zeroForOne,
            ____amountIn,
            _sqrtPriceLimitX96,
            abi.encode(___tokenIn, __tokenOut, __fee)
        );

        int256 _amountOutSigned = zeroForOne ? -amount1 : -amount0;
        uint _amountOut = uint256(_amountOutSigned);

        // If slippage protection requested, if we didn't precompute expected, compute now from quote
        if (_slippageBps > 0 && amountOutExpected == 0) {
            amountOutExpected = quoteV3View(___tokenIn, __tokenOut, __fee, uint256(____amountIn), _sqrtPriceLimitX96);
            minAmountOut = (amountOutExpected * (10000 - _slippageBps)) / 10000;
        }

        emit SwapDebugV3(_poolUsed, ___tokenIn, __tokenOut, __fee, uint(____amountIn), amountOutExpected, minAmountOut, IUniswapV3Pool(_poolUsed).liquidity());

        if (_slippageBps > 0 && _amountOut < minAmountOut) revert SlippageExceeded(_amountOut, minAmountOut);
